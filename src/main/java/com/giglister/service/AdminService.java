...
public class AdminService {
    ...
    public List<DuplicateCandidate> possibleDuplicates() {
        ...
        for (int i = 0; i < bands.size(); i++) {
            for (int j = i + 1; j < bands.size(); j++) {
                ...
                if (sim >= 0.8 || (sim >= 0.6 && sameCity)) {
                    result.add(new DuplicateCandidate(com.giglister.domain.enums.EntityType.BAND,
                            bands.get(j).getId(), bands.get(j).getName(), bands.get(j).getCity(), sim));
                }
            }
        }
        ...
        for (int i = 0; i < locations.size(); i++) {
            for (int j = i + 1; j < locations.size(); j++) {
                ...
                if (sim >= 0.8 || (sim >= 0.6 && sameCity)) {
                    result.add(new DuplicateCandidate(com.giglister.domain.enums.EntityType.LOCATION,
                            locations.get(j).getId(), locations.get(j).getName(), locations.get(j).getCity(), sim));
                }
            }
        }
        return result;
