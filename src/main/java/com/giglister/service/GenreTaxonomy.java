 ...
 * single-word base genres; a filter matches any band whose genre text CONTAINS it as a
 * substring (case-insensitive), so "Stonerpunk" surfaces under both "Stoner" and "Punk"
 * without either needing to be its own literal genre anywhere. Deliberately kept to plain,
 * single words (no "Post-Punk"/"Hip-Hop") so a compound like "Postpunk" or "Post-Punk" still
 * matches "Punk" - splitting further than that would need real NLP, not string matching.
 */
public final class GenreTaxonomy {
    ...
    public static final List<String> BASE_GENRES = List.of(
            ...
            "Metalcore", "Noise", "Pop", "Progressive", "Psychedelic", "Punk", "Rap", "Reggae",
            "Rock", "Shoegaze", "Ska", "Soul", "Stoner", "Synth", "Techno", "Thrash", "Trance",
            "Wave", "World"
    );
