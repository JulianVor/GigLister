  ...
  const backgroundControls = <div className="space-y-4">
    ...
    {!draft.background.image && <>
      <div className="space-y-2">
        <p className="font-meta text-sm">Farbverlauf</p>
        {draft.background.colors.map((stop, i) => <div key={i} className="flex items-center gap-2">
          <input aria-label={`Farbe ${i + 1}`} type="color" className="h-9 w-9 flex-none cursor-pointer border border-line" value={stop.color} onChange={e => change({ ...draft, background: { ...draft.background, colors: draft.background.colors.map((s, si) => si === i ? { ...s, color: e.target.value } : s) } })} />
          <input aria-label={`Position Farbe ${i + 1}`} type="range" min="0" max="100" step="1" className="flex-1 accent-[var(--accent)]" value={Math.round(stop.stop * 100)} onChange={e => change({ ...draft, background: { ...draft.background, colors: draft.background.colors.map((s, si) => si === i ? { ...s, stop: Number(e.target.value) / 100 } : s) } })} />
          <span className="w-9 flex-none text-right font-meta text-xs text-muted">{Math.round(stop.stop * 100)}%</span>
          {draft.background.colors.length > 2 && <button type="button" aria-label={`Farbe ${i + 1} entfernen`} className="flex-none font-meta text-muted hover:text-fg" onClick={() => change({ ...draft, background: { ...draft.background, colors: draft.background.colors.filter((_, si) => si !== i) } })}>×</button>}
        </div>)}
        <button type="button" className={button} disabled={draft.background.colors.length >= 8} onClick={() => change({ ...draft, background: { ...draft.background, colors: [...draft.background.colors, { color: "#888888", stop: .5 }] } })}>+ Farbe hinzufügen</button>
      </div>
      ...
      <label className="block font-meta text-sm">Chaos · {draft.background.patternChaos.toFixed(1)}<input aria-label="Chaos" className="mt-2 w-full accent-[var(--accent)]" type="range" min="0" max="5" step="0.1" value={draft.background.patternChaos} onChange={e => change({ ...draft, background: { ...draft.background, patternChaos: Number(e.target.value) } })} /></label>
      ...
      <button className={button} onClick={() => { const palettes = [["#171f2c", "#c84b24"], ["#0b3535", "#9a7b27"], ["#261533", "#ad355a"], ["#111111", "#626262"]]; const picked = palettes[Math.floor(Math.random() * palettes.length)]; change({ ...draft, background: { ...draft.background, colors: picked.map((color, i) => ({ color, stop: i })), seed: Math.floor(Math.random() * 1e6) } }); }}>Neue Farbstimmung</button>
    </>}
