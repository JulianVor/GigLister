...
export default async function AdminEventSeriesPage({
  ...
  return (
    <div>
      ...
      <div className="mt-6">
        <AdminSearchForm action="/admin/festivals" query={q} placeholder="Suche nach Name …" />
      </div>

      <p className="mt-4 font-meta text-xs text-muted">{page.totalElements} Festivals</p>

      <div className="mt-2 divide-y divide-line border-y border-line">
        {page.content.length === 0 ? (
          <p className="py-6 font-meta text-sm text-muted">Keine Festivals gefunden.</p>
        ) : (
          page.content.map((series) => (
            <div key={series.id} className="flex items-center justify-between gap-4 py-3">
              <Link href={`/festivals/${series.id}`} className="font-display text-lg hover:text-accent">
                {series.name}
              </Link>
              <div className="flex items-center gap-3">
                ...
                <Link href={`/festivals/${series.id}/bearbeiten`} className="font-meta text-sm text-accent hover:underline">
                  Bearbeiten
                </Link>
              </div>
            </div>
